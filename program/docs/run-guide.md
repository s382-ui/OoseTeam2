...
이 문서는 `program` 폴더의 연구실 안전관리 시스템을 실제로 실행하는 방법을 정리한다. 현재 프로젝트는 Java 21 기반이며, 기본 실행 화면은 JavaFX/FXML UI다. 빌드 도구는 Gradle을 사용한다.
...
현재 기본 실행은 JavaFX 창을 열며, 왼쪽 메뉴의 `안전교육 관리`를 선택해 안전교육관리 화면으로 이동한다.
