...
public record EducationRecord(
    ...
    public String summary() {
        return "이수결과ID=%s, 연구활동종사자ID=%s, 개설ID=%s, 학습결과ID=%s, 일지ID=%s, 이수일=%s, 인정시간=%d, 상태=%s, 수동등록=%s".formatted(
            completionResultId, researcherId, openingId, learningResultId, logId, completionDate, recognizedHours, completionStatus, manualRegistered);
    }
}
